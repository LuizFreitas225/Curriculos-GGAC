...
public class CurriculoRepository extends Repository<Curriculo> {
	...
	@SuppressWarnings("unchecked")
	public List<Curriculo> findByDescricao(String descricao, Integer maxResults, Usuario usuario)
			throws RepositoryException {
		try {
			EntityManager em = getEntityManager();
			// JPQL ou SQL
			Query query = em
					.createQuery("SELECT c FROM Curriculo c " + "WHERE upper(c.descricao)"
							+ " LIKE upper(:descricao) "
							+ " AND :usuario = c.usuario  " + "ORDER BY c.descricao");
			query.setParameter("descricao", "%" + descricao + "%");
